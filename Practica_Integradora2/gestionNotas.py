...
def nota_mas_alta(materiasCargadas):
    for alumno, materias in materiasCargadas:
        ...
        for materia in materias:
            if materia[3] > nota_top:   
                nota_top = materia[3]   
                materia_top = materia[0] 

        print(f"La materia con la nota mas alta es {materia_top} el promedio es {nota_top}")
...
def pedirNotas(materias):
    for materia in materias:
        while True:
            nota1 = pedir_entero(f"Ingrese Nota 1 para {materia[0]}: ")
            if 0 <= nota1 <= 10:
                break
        ...
        while True:
            nota2 = pedir_entero(f"Ingrese Nota 2 para {materia[0]}: ")
            if 0 <= nota2 <= 10:
                break
            print("Ingrese una nota valida entre 0 y 10")
        materia[2] = nota2
...
for alumno, materias in materiasCargadas:
    ...
    nota_mas_alta(materiasCargadas)
